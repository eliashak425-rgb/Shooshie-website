...
vec3 render(vec2 st, vec2 mouse) {
    ...
    // Neon purple color
    vec3 color = vec3(0.65, 0.25, 0.95);
    
    // Add glow effect
    float glow = shape * 1.5;
    vec3 glowColor = vec3(0.55, 0.15, 0.92) * glow;
    
    float dimming = 1.0 - mouseInfluence * 0.2;
    color = (color * shape + glowColor * 0.3) * dimming;
...
export default function GeometricBlurMesh() {
    ...
    // Visibility observer - pause rendering when not visible
    useEffect(() => {
    ...
    }, [currentShape, isVisible]);

    return (
        <div ref={containerRef} className="relative w-full h-[500px] cursor-pointer overflow-hidden rounded-lg border border-purple-500/20">
